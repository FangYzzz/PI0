...
class OculusReader:
    ...
    def get_valid_transforms_and_buttons(self):
        while True:
            ...
            # print(transforms)
            if "l" in transforms or "r" in transforms:  # and
...
class Oculus_mobile():
    ...
    * gripper for each arms: a binary command to open or close the gripper.
    ...
    def reset(self):
        ...
        self._close_gripper_left = False
        self._close_gripper_right = False
        self._save_episode_right = False
    ...
    def advance(self):
        ...
        return (
            ...
            buttons["RG"], # button_RG: move
            buttons["A"], # Right gripper grasp
            buttons["B"], # Right gripper release
            buttons["RJ"], # save_episode
            buttons["RTr"]
        )
    ...
    #    [ 0.       ,  0.       ,  0.       ,  1.       ]])}, {'A': False, 'B': False, 'RThU': True, 'RJ': False, 'RG': False, 'RTr': False, 'X': False, 'Y': False, 'LThU': True, 'LJ': False, 'LG': False, 'LTr': False, 'leftJS': (0.0, 0.0), 'leftTrig': (0.0,), 'leftGrip': (0.0,), 'rightJS': (0.0, 0.0), 'rightTrig': (0.0,), 'rightGrip': (0.0,)})
        ...
        # if buttons["B"] and not self._prev_B_state:
        #     self._close_gripper_right = not self._close_gripper_right
        # # Update the previous state for the next cycle
        # self._prev_B_state = buttons["B"]

        # # save_episode
        # if buttons["A"] and not self._prev_A_state:
        #     self._save_episode_right = not self._save_episode_right
        # # Update the previous state for the next cycle
        # self._prev_A_state = buttons["A"]
        ...
        #     # self._close_gripper_left,  # Left gripper
        #     np.concatenate([self._delta_pos_right, self._delta_rot_right]),  # Right arm
        #     self._close_gripper_right,  # button_B: Right gripper
        #     buttons["RG"], # button_RG: move
        #     self._save_episode_right, # button_A: save_episode
        #     self._prev_A_state # button_A
        #     # buttons["A"],
        #     # buttons["B"] # button_B: save_episode
        # )