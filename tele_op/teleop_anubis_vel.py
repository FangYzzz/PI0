...
REPO_NAME = "lerobot_velocity_joint" # (with pause)  # lerobot_pickup_pepper_absolute_joint(without pause)
            ...
            if is_recording and (RG or A or B) and last_joint_state is not None:
                ...
                if (now - last_save_time >= DT):
                    ...
                    frame = {
                        ...
                        "actions": np.concatenate([current_joint_state-last_joint_state, [gripper_state_]]).astype(np.float32),
                        "task": str(current_task),
                    }
