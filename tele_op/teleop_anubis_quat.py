...
def release_gripper(gripper):
    speed = 0.05  # 0.02[m/s]
    # gripper.open(speed)
    gripper.move_async(0.085, speed)
    return 0
...
def main():
    ...
    teleop_interface = Oculus_mobile(pos_sensitivity=10, rot_sensitivity=1,) # 1.0, 0.8
    ...
    while True:
        ...
        if is_recording and (RG or RTr or B) and last_joint_state is not None:
            ...
            sign = np.where(eef_q[..., 3:4] < 0, -1.0, 1.0)
            eef_q = eef_q * sign
            current_eef_state = np.concatenate([eef_t, eef_q])
            # print(current_eef_state.shape)
            # print(now)
